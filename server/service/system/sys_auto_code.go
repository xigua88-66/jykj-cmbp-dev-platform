...
import (
	...
	ast2 "jykj-cmbp-dev-platform/server/utils/ast"

	cp "github.com/otiai10/copy"
	"go.uber.org/zap"
	"jykj-cmbp-dev-platform/server/resource/autocode_template/subcontract"
...
func Init(Package string) {
	injectionPaths = []injectionMeta{
		{
			path: filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SApi, Package), "enter.go"),
			funcName:    "ApiGroup",
			structNameF: "%sApi",
		},
		{
			path: filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SRouter, Package), "enter.go"),
			funcName:    "RouterGroup",
			structNameF: "%sRouter",
		},
		{
			path: filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SService, Package), "enter.go"),
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			path: filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, "service", "enter.go"),
			importCodeF:  "jykj-cmbp-dev-platform/server/%s/%s",
			packageNameF: "%s",
		...
		packageRouterName: {
			path: filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, "router", "enter.go"),
			importCodeF:  "jykj-cmbp-dev-platform/server/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, "api/v1", "enter.go"),
			importCodeF:  "jykj-cmbp-dev-platform/server/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...string) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
...

		{
			// 在gorm.go 注入 自动迁移
			path := filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, global.CMBP_CONFIG.AutoCode.SInitialize, "gorm.go")
...

		{
			// router.go 注入 自动迁移
			path := filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, global.CMBP_CONFIG.AutoCode.SInitialize, "router.go")
...
func (autoCodeService *AutoCodeService) DropTable(BusinessDb, tableName string) error {
	if BusinessDb != "" {
		...
		return global.CMBP_DB.Exec("DROP TABLE " + tableName).Error
	}
}
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		if strings.Contains(fileSlice[n-2], "router") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Server,
				fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SRouter, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "api") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SApi, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "service") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SService, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "model") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SModel, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "request") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SRequest, data.autoPackage), base)
		}
	} else if strings.Contains(fileSlice[1], "web") {
		if strings.Contains(fileSlice[n-1], "js") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Web, global.CMBP_CONFIG.AutoCode.WApi, base)
		} else if strings.Contains(fileSlice[n-2], "form") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Web, global.CMBP_CONFIG.AutoCode.WForm, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
		} else if strings.Contains(fileSlice[n-2], "table") {
			data.autoMoveFilePath = filepath.Join(global.CMBP_CONFIG.AutoCode.Root,
				global.CMBP_CONFIG.AutoCode.Web, global.CMBP_CONFIG.AutoCode.WTable, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), base)
		}
	}
}
...
func (autoCodeService *AutoCodeService) AutoCreateApi(a *system.AutoCodeStruct) (ids []string, err error) {
	...
	err = global.CMBP_DB.Transaction(func(tx *gorm.DB) error {
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for i, s := range pendingTemp {
		pendingTemp[i].path = filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Server, filepath.Clean(fmt.Sprintf(s.path, packageName)))
	}
	// 选择模板
...
func (autoCodeService *AutoCodeService) CreatePlug(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[2], "tpl") < 0 {
			dirPath := filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SPlug, plug.Snake+"/"+pathArr[2]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Server, fmt.Sprintf(global.CMBP_CONFIG.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugPath):len(tpl)-4]))
...
func (autoCodeService *AutoCodeService) InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
	...
	if len(serverPlugin) != 0 {
		err = installation(serverPlugin, global.CMBP_CONFIG.AutoCode.Server, global.CMBP_CONFIG.AutoCode.Server)
		if err != nil {
	...
	if len(webPlugin) != 0 {
		err = installation(webPlugin, global.CMBP_CONFIG.AutoCode.Server, global.CMBP_CONFIG.AutoCode.Web)
		if err != nil {
			return webIndex, serverIndex, err
		}
	}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.ToSlash(global.CMBP_CONFIG.AutoCode.Root + formPath + "/" + path)
	var to = filepath.ToSlash(global.CMBP_CONFIG.AutoCode.Root + toPath + "/plugin/")
...
func (autoCodeService *AutoCodeService) PubPlug(plugName string) (zipPath string, err error) {
	...
	webPath := filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Web, "plugin", plugName)
	serverPath := filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Server, "plugin", plugName)
	// 创建一个新的zip文件
	...
	return filepath.Join(global.CMBP_CONFIG.AutoCode.Root, global.CMBP_CONFIG.AutoCode.Server, fileName), nil
}
