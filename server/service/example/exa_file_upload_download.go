...
func (e *FileUploadAndDownloadService) Upload(file example.ExaFileUploadAndDownload) error {
	return global.CMBP_DB.Create(&file).Error
}
...
func (e *FileUploadAndDownloadService) FindFile(id string) (example.ExaFileUploadAndDownload, error) {
	var file example.ExaFileUploadAndDownload
	err := global.CMBP_DB.Where("id = ?", id).First(&file).Error
	return file, err
}
...
func (e *FileUploadAndDownloadService) DeleteFile(file example.ExaFileUploadAndDownload) (err error) {
	...
	err = global.CMBP_DB.Where("id = ?", file.ID).Unscoped().Delete(&file).Error
	return err
}
...
func (e *FileUploadAndDownloadService) EditFileName(file example.ExaFileUploadAndDownload) (err error) {
	var fileFromDb example.ExaFileUploadAndDownload
	return global.CMBP_DB.Where("id = ?", file.ID).First(&fileFromDb).Update("name", file.Name).Error
}
...
func (e *FileUploadAndDownloadService) GetFileRecordInfoList(info request.PageInfo) (list interface{}, total int64, err error) {
	...
	db := global.CMBP_DB.Model(&example.ExaFileUploadAndDownload{})
