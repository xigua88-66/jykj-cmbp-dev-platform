...
type Register struct {
	...
	AuthorityId  string   `json:"authorityId" swaggertype:"string" example:"int 角色id"`
	Enable       int      `json:"enable" swaggertype:"string" example:"int 是否启用"`
	AuthorityIds []string `json:"authorityIds" swaggertype:"string" example:"[]uint 角色id"`
...
type Login struct {
	...
	ImgID    string `json:"img_id"`   // 验证码ID
}

// Modify password structure
type ChangePasswordReq struct {
	ID          uint   `json:"-"`            // 从 JWT 中提取 user id，避免越权
	OldPassword string `json:"old_password"` // 密码
	NewPassword string `json:"new_password"` // 新密码
	Username    string `json:"username"`
	DingAccount string `json:"ding_account"`
}
