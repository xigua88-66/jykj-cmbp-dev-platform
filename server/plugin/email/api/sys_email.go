...
import (
	...
	email_response "jykj-cmbp-dev-platform/server/plugin/email/model/response"
	"jykj-cmbp-dev-platform/server/plugin/email/service"
)
...
func (s *EmailApi) EmailTest(c *gin.Context) {
	...
	if err != nil {
		global.CMBP_LOG.Error("发送失败!", zap.Error(err))
